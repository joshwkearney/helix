...
namespace Attempt12.Language {
    public class LanguageInterpreter {
        ...
        public LanguageInterpreter() {
            ...
            scope = scope.SetMacro(
                data => {
                    ...
                    foreach (var pair in list[0].AsDictionary()) {
                        ...
                        newVars[pair.Key] = Data.From(new Dictionary<Data, Data>() {
                    ...
                    return Data.From(new List<Data>() { "@", result });
                },
                "define"
            );
            ...
            scope = scope.SetMacro(
                data => {
                    var list = data["and"]
                        .AsList()
                        .Select(x => this.Interpret(x))
                        .Aggregate((x, y) => (x.IsTruthy && y.IsTruthy) ? Data.From(1) : Data.From(0));
            ...
            scope = scope.SetMacro(
                data => {                  
                    return Data.From(new List<Data>() {
            ...
            scope = scope.SetMacro(
                data => {
                    ...
                    if (!next.IsTruthy) {
                        next = Data.From(new Dictionary<Data, Data>());
                    }
                    ...
                    if (accessors.Count == 0) {
                        ...
                        foreach (var accessor in accessors.Skip(1).Take(accessors.Count - 2)) {
                            if (current.ContainsKey(accessor)) {
                                ...
                                current[accessor] = Data.From(empty);
                                current = empty; 
                            }
                        }
            ...
            scope = scope.SetMacro(
                data => {
                    ...
                    return Data.From(new List<Data>() { "@", current });
                },
                "get"
            );
            ...
            scope = scope.SetMacro(
                data => {
                    ...
                    return Data.From(new List<Data>() {
                        "@",
                        Data.From(new Dictionary<Data, Data>() {
                            { "args", Data.From(args) },
                            { "body", body },
                            { "scope", Data.From(this.variables1.Peek()) }
                        })
                    });
                },
                "function"
            ...
            scope = scope.SetMacro(
                data => {
                    ...
                    foreach (var (name, value) in argNames.Zip(argValues, (x, y) => (name: x, value: y))) {
                        funcScope[name] = Data.From(new Dictionary<Data, Data>() {
                            { "value", value }
                        });
                    }
            ...
            scope = scope.SetMacro(
                data => {
                    var code = new List<Data>() {
                        "call",
                        Data.From(new List<Data>() { "@", this.currentFunction.Peek() })
                    };

                    var list = data["recurse"].AsList();
                    code.AddRange(list);

                    var result = Data.From(code);
                ...
                data => {
                    ...
                    newScope = newScope.SetMacro(
                        macroData => this.Interpret(
                            Data.From(new List<Data>() {
                                "call",
                                Data.From(new List<Data>() { "@", func }),
                                Data.From(new List<Data>() { "@", Data.From(macroData) }),
                            }
                        )),
                        names
                    );
